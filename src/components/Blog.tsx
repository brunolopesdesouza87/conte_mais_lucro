...
const Blog: React.FC = () => {
  const latestPosts = [].slice(0, 6);

  return (
    <section
      id="blog"
      className="py-32 bg-brand-black text-white border-t border-white/5 relative overflow-hidden"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {latestPosts.map((post, i) => (
            <Link
              to={`/blog/${post.id}`}
              key={post.id}
              className="group cursor-pointer flex flex-col h-full bg-brand-darkGray/50 border border-white/5 hover:border-brand-red/30 transition-all duration-700 reveal"
            ...
            >
              <div className="relative aspect-[16/10] overflow-hidden">
                <img
                  src={post.image}
                  alt={post.title}
                  className="w-full h-full object-cover transition-all duration-1000 group-hover:scale-110 group-hover:rotate-1 opacity-60 group-hover:opacity-100"
                />
              ...
              <div className="p-10 flex flex-col flex-grow relative">
                ...
                <p className="text-white/40 text-sm leading-relaxed mb-10 line-clamp-3 group-hover:text-white/60 transition-colors">
                  {post.excerpt}
                </p>
                ...
                <div className="mt-auto pt-8 border-t border-white/5 flex items-center justify-between">
                  ...
                  <ArrowUpRight
                    className="text-brand-red group-hover:rotate-45 transition-transform duration-500"
                    size={18}
                  />
                </div>
              </div>
